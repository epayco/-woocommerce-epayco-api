(function($) {
    const fieldMapsBilling = {
        'woo-epayco-creditcard': {
            '#billing-address_1': 'input[name="epayco_creditcard[address]"]',
            '#billing-phone': 'input[name="epayco_creditcard[cellphone]"]',
            '#billing-city': 'input[name="epayco_creditcard[country]"]',
            '#email': 'input[data-epayco="card[email]"]'
        },
        'woo-epayco-pse': {
            '#billing-address_1': 'input[name="epayco_pse[address]"]',
            '#billing-phone': 'input[name="epayco_pse[cellphone]"]',
            '#billing-city': 'input[name="epayco_pse[country]"]',
            '#email': 'input[name="epayco_pse[email]"]'
        },
        'woo-epayco-daviplata': {
            '#billing-phone': 'input[name="epayco_daviplata[cellphone]"]',
            '#email': 'input[name="epayco_daviplata[email]"]'
        },
        'woo-epayco-ticket': {
            '#billing-phone': 'input[name="epayco_ticket[cellphone]"]',
            '#billing-city': 'input[name="epayco_ticket[country]"]',
            '#email': 'input[name="epayco_ticket[email]"]'
        },
        'woo-epayco-subscription': {
            '#billing-address_1': 'input[name="epayco_subscription[address]"]',
            '#billing-phone': 'input[name="epayco_subscription[cellphone]"]',
            '#email': 'input[name="epayco_subscription[email]"]'
        },
    };

    const fieldMapsShipping = {
        'woo-epayco-creditcard': {
            '#shipping-address_1': 'input[name="epayco_creditcard[address]"]',
            '#shipping-phone': 'input[name="epayco_creditcard[cellphone]"]',
            '#shipping-city': 'input[name="epayco_creditcard[country]"]',
            '#email': 'input[data-epayco="card[email]"]' 
        },
        'woo-epayco-pse': {
            '#shipping-address_1': 'input[name="epayco_pse[address]"]',
            '#shipping-phone': 'input[name="epayco_pse[cellphone]"]',
            '#shipping-city': 'input[name="epayco_pse[country]"]',
            '#email': 'input[name="epayco_pse[email]"]'
        },
        'woo-epayco-daviplata': {
            '#shipping-phone': 'input[name="epayco_daviplata[cellphone]"]',
            '#email': 'input[name="epayco_daviplata[email]"]'
        },
        'woo-epayco-ticket': {
            '#shipping-phone': 'input[name="epayco_ticket[cellphone]"]',
            '#shipping-city': 'input[name="epayco_ticket[country]"]',
            '#email': 'input[name="epayco_ticket[email]"]'
        },
        'woo-epayco-subscription': {
            '#shipping-address_1': 'input[name="epayco_subscription[address]"]',
            '#shipping-phone': 'input[name="epayco_subscription[cellphone]"]',
            '#email': 'input[name="epayco_subscription[email]"]'
        },
    };

    function getFieldMapSet() {
        return $('#shipping-first_name').length ? fieldMapsShipping : fieldMapsBilling;
    }

    function autofillName(method) {
        const useShipping = $('#shipping-first_name').length > 0;
        const firstName = $(useShipping ? '#shipping-first_name' : '#billing-first_name').val() || '';
        const lastName = $(useShipping ? '#shipping-last_name' : '#billing-last_name').val() || '';
        const fullName = (firstName + ' ' + lastName).trim();

        let selector = '';
        switch (method) {
            case 'woo-epayco-creditcard':
                selector = 'input[name="epayco_creditcard[name]"]'; break;
            case 'woo-epayco-pse':
                selector = 'input[name="epayco_pse[name]"]'; break;
            case 'woo-epayco-daviplata':
                selector = 'input[name="epayco_daviplata[name]"]'; break;
            case 'woo-epayco-ticket':
                selector = 'input[name="epayco_ticket[name]"]'; break;
            case 'woo-epayco-subscription':
                selector = 'input[name="epayco_subscription[name]"]'; break;
        }

        if (selector && $(selector).length) {
            $(selector).val(fullName).trigger('input');
        }
    }

    function autofillEpaycoFields(method) {
        autofillName(method);
        const fieldMap = getFieldMapSet()[method];
        if (!fieldMap) return;

        $.each(fieldMap, function(wcSelector, epaycoSelector) {
            const wcValue = $(wcSelector).val();
            if (wcValue !== undefined && $(epaycoSelector).length) {
                $(epaycoSelector).val(wcValue).trigger('input');
            }
        });
    }

    function getSelectedMethod() {
        return $('input[name="radio-control-wc-payment-method-options"]:checked').val();
    }

    function handleAutofill() {
        const selected = getSelectedMethod();
        if (!selected) return;

        const fieldMap = getFieldMapSet()[selected];
        if (!fieldMap) return;

        const allFieldsReady = Object.keys(fieldMap).every((selector) => $(selector).length > 0);

        if (!allFieldsReady) {
            setTimeout(handleAutofill, 300);
        } else {
            autofillEpaycoFields(selected);
        }
    }

    $(document).ready(function() {
        setTimeout(handleAutofill, 500);
        $(document.body).on('change', 'input[name="radio-control-wc-payment-method-options"]', handleAutofill);
        $(document.body).on('updated_checkout', handleAutofill);
    });
})(jQuery);

