                ...
                return n.setAttribute("name", this.getAttribute("input-name-epayco")),
                    ...
                    n.addEventListener("input", (() => {
                        ...
                        switch (i.querySelector("select").value) {
                            ...
                            case "CE":
                                // n.value = n.value.replace(/[^A-Za-z1-9]/g, '');
                                n.value = n.value.replace(/^0+/, '');
                    ...
                    , n.addEventListener("focusout", (() => {
                        ...
                        // Validación específica para el campo NIT
                        if (i.querySelector("select").value === "NIT") {
                            const nitRegex = /^(?!.*\s)[1-9]\d{6,7}(?:-\d)?$/;
                            ...
                            // Validar formato final del NIT - permitir guión al final sin dígito
                            const nitValue = n.value.trim();
                            const nitWithOptionalDigit = /^[1-9]\d{6,7}(?:-\d)?$/;
                            const nitWithGuionOnly = /^[1-9]\d{6,7}-$/;

                            if (!nitWithOptionalDigit.test(nitValue) && !nitWithGuionOnly.test(nitValue)) {
                                // Marcar como error si el formato no es válido
                                i.querySelector("select").parentElement.classList.add("ep-error");
                                t.querySelector("input").parentElement.classList.add("ep-error");
                                ...
                                if (helperDiv) {
                                    helperDiv.style.display = "flex";
                                    helperDiv.querySelector('.ep-helper-message').textContent =
                                        "NIT: Formato inválido (7-11 dígitos, guión opcional)";
                                }
                        ...
                        // Validación específica para el campo CE

                        if (i.querySelector("select").value === "CE") {

                            const ceRegex = /^(?!.*\s)[A-Za-z1-9]+$/;
                            ...
                            if (!ceRegex.test(n.value.trim())) {
                                ...
                                n.removeAttribute("name");
                                n.setAttribute("data-invalid", "true");
                                n.setCustomValidity("[1-9]: Debe contener al menos un dígito del 1 al 9.");
                                return;
                            } else {
                                // Validar cantidad de letras y dígitos
                                const letras = (n.value.match(/[A-Za-z]/g) || []).length;
                                const digitos = (n.value.match(/[1-9]/g) || []).length;
                        ...
                        // Validación específica para el campo SSN

                        if (i.querySelector("select").value === "SSN") {
                            
                            const ssnRegex = /^(?!.*\s)\d{3}-?\d{2}-?\d{4}$/;
                        ...
                        // Validación específica para el campo RFC

                        if (i.querySelector("select").value === "RFC") {
                          
                            const rfcRegex = /^[a-zA-Z]{3,4}\d{6}[a-zA-Z0-9]{3}$/;
