    ...
    public function getProcessedStatus(\WC_Order $order, $data): string
    {
        ...
        if (!empty($data['payments_details'])) {
            ...
            foreach ($data['payments_details'] as $payment) {
                ...
                $this->updateMeta($order, 'ePayco - ' . $payment['id'] . ' - payment_type', $payment['payment_type_id']);

                if (strpos($payment['payment_type_id'], 'card') !== false) {
                    $this->updateMeta($order, 'ePayco - ' . $payment['id'] . ' - installments', $payment['payment_method_info']['installments']);
                    $this->updateMeta($order, 'ePayco - ' . $payment['id'] . ' - installment_amount', $payment['payment_method_info']['installment_amount']);
                    $this->updateMeta($order, 'ePayco - ' . $payment['id'] . ' - transaction_amount', $payment['total_amount']);
                    $this->updateMeta($order, 'ePayco - ' . $payment['id'] . ' - total_paid_amount', $payment['paid_amount']);
                    $this->updateMeta($order, 'ePayco - ' . $payment['id'] . ' - card_last_four_digits', $payment['payment_method_info']['last_four_digits']);
                }
            }

            if (count($payment_ids) != 0) {
                $this->updateMeta($order, '_Epayco_Payment_IDs', implode(', ', $payment_ids));
            }
        }
