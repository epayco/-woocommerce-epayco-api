...
class Notices
{
     ...
     * @const
     */
    private const NONCE_ID = 'ep_notices_dismiss';
    ...
    public function __construct(
        ...
        $this->endpoints->registerAjaxEndpoint('ep_review_notice_dismiss', [$this, 'reviewNoticeDismiss']);
        $this->endpoints->registerAjaxEndpoint('ep_saved_cards_notice_dismiss', [$this, 'savedCardsDismiss']);
    ...
    public function loadAdminNoticesCss(): void
    {
        if (is_admin()) {
            $this->scripts->registerAdminStyle(
                ...
                $this->url->getPluginFileUrl('assets/css/admin/ep-admin-notices', '.css')
            );
        }
    }
    ...
    public function loadAdminNoticesJs(): void
    {
        if (is_admin()) {
            $this->scripts->registerAdminScript(
                'woocommerce_epayco_admin_notice_js',
                $this->url->getPluginFileUrl('assets/js/admin/ep-admin-notices', '.js'),
                [
    ...
    public function insertDismissibleNotices(): void
    {
        ...
        if (
            ...
            add_action(
                'admin_notices',
                function () {
                    $cardIcon   = $this->url->getPluginFileUrl('assets/images/icons/logo', '.png', true);
    ...
    public function shouldShowNotices(): bool
    {
        return is_admin() &&
            (
                $this->url->validateSection('epayco')
    ...
    public function adminNoticeMissPix(): void
    {
        add_action(
            ...
            function () {
                ...
                $urlLink  = $this->links['epayco_pix_config'];
    ...
    public function storePendingStatusNotice($orderStatus, string $urlReceived, string $checkoutType, string $linkText): void
    {
        $message = "
            <p>$orderStatus</p>
            <a id='ep_pending_payment_button' class='button' href='$urlReceived' data-mp-checkout-type='woo-epayco-$checkoutType'>
                $linkText
            </a>
        ";
    ...
    public function storeRejectedStatusNotice(string $noticeTitle, string $orderStatus, string $urlReceived, string $checkoutType, string $linkText): void
    {
        $message = "
            ...
            <a id='ep_failed_payment_button' class='button' href='$urlReceived' data-ep-checkout-type='woo-epayco-$checkoutType'>
                $linkText
            </a>
        ";
