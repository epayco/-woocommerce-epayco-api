...
final class OrderStatus
{
    ...
    private function approvedFlow(array $data, \WC_Order $order, string $usedGateway): void
    {
        ...
        if ($status === 'pending' || $status === 'on-hold' || $status === 'failed') {
            $order->add_order_note('ePayco: ' . $this->translations['payment_approved']);
    ...
    private function pendingFlow(array $data, \WC_Order $order, string $usedGateway): void
    {
        if ($this->canUpdateOrderStatus($order)) {
            ...
            switch ($usedGateway) {
                case 'Sdk\Woocommerce\Gateways\PixGateway':
                    ...
                    $order->add_order_note('ePayco: ' . $this->translations['pending_pix']);
                    $order->add_order_note('ePayco: ' . $this->translations['pending_pix'], 1);
                    break;

                case 'Epayco\Woocommerce\Gateways\TicketGateway':
                    $notes = $order->get_customer_order_notes();
                    ...
                    $order->add_order_note('ePayco: ' . $this->translations['pending_ticket']);
                    $order->add_order_note('ePayco: ' . $this->translations['pending_ticket'], 1);
                    break;

                default:
                    $order->add_order_note('ePayco: ' . $this->translations['pending']);
                    break;
            }
        } else {
    ...
    private function inProcessFlow(array $data, \WC_Order $order): void
    {
        if ($this->canUpdateOrderStatus($order)) {
            $order->update_status(
                ...
                'ePayco: ' . $this->translations['in_process']
            );
        } else {
    ...
    private function rejectedFlow(array $data, \WC_Order $order): void
    {
        if ($this->canUpdateOrderStatus($order)) {
            $order->update_status(
                ...
                'ePayco: ' . $this->translations['rejected']
            );
        } else {
    ...
    private function refundedFlow(array $data, \WC_Order $order): void
    {
        if ($this->isPartialRefund($data)) {
            ...
            $order->add_order_note('ePayco: ' . $this->translations['partial_refunded'] . $refund_amount);
            return;
        }

        $order->update_status(
            self::mapMpStatusToWoocommerceStatus('refunded'),
            'ePayco: ' . $this->translations['refunded']
        );
    }
    ...
    private function cancelledFlow(array $data, \WC_Order $order): void
    {
        if ($this->canUpdateOrderStatus($order)) {
            $order->update_status(
                ...
                'ePayco: ' . $this->translations['cancelled']
            );
        } else {
    ...
    private function inMediationFlow(\WC_Order $order): void
    {
        ...
        $order->add_order_note('ePayco: ' . $this->translations['in_mediation']);
    }
    ...
    private function chargedBackFlow(\WC_Order $order): void
    {
        ...
        $order->add_order_note('ePayco: ' . $this->translations['charged_back']);
    }
    ...
    protected function validateOrderNoteType(array $data, \WC_Order $order, string $status): void
    {
        ...
        $order->add_order_note("Sdk: {$this->translations['validate_order_1']} $paymentId {$this->translations['validate_order_1']} $status");
    }
